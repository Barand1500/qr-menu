...
import { DEFAULT_ALIVE_CONFIG, type AliveThemeConfig } from '@/lib/menuAliveConfig';

export default function AliveThemeSettingsModal({
  open,
  initial,
  saving,
  onClose,
  ...
  open: boolean;
  initial: AliveThemeConfig;
  saving?: boolean;
  onClose: () => void;
  onSave: (config: AliveThemeConfig) => Promise<void> | void;
}) {
  const [draft, setDraft] = useState<AliveThemeConfig>(initial);
  ...
  return (
    <div
      className="linear-settings alive-settings"
      role="dialog"
      aria-modal="true"
      aria-labelledby="alive-settings-title"
    >
      <button type="button" className="linear-settings__scrim" aria-label="Kapat" onClick={onClose} />
      <div className="linear-settings__panel">
        <header className="linear-settings__head linear-settings__head--alive">
          <div>
            <p className="linear-settings__eyebrow">
              <Settings2 className="w-3.5 h-3.5" /> Canlı
            </p>
            <h2 id="alive-settings-title">Tema ayarları</h2>
        ...
        <div className="linear-settings__body alive-settings__body">
          <button
            type="button"
          ...
          <p className="alive-settings__hint">
            Kapalıysa ürün detayında boy / ekstra / istek seçenekleri gizlenir. Varsayılan:{' '}
            {DEFAULT_ALIVE_CONFIG.variantsEnabled ? 'açık' : 'kapalı'}.
          </p>
          ...
          <p className="alive-settings__hint">
            ...
            {DEFAULT_ALIVE_CONFIG.cartEnabled ? 'açık' : 'kapalı'}.
          </p>
