...
function CatalogEditorPanel({
    ...
    onChange(
      items.map((item) =>
        item.id === id
          ? {
              ...item,
              label: { tr: name, en: name, ru: name, ar: name },
            }
          : item
      )
    );
  ...
  function addItem() {
    ...
    onChange([
      ...items,
      { id, label: { tr: name, en: name, ru: name, ar: name } },
    ]);
    setNewValue('');
    setAdding(false);
  }
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 shrink-0 self-center">
            <button
              type="button"
              onClick={addItem}
              className="h-9 px-3 rounded-lg text-xs font-semibold transition hover:opacity-90 active:scale-[0.98]"
              style={{
