...
import { DEFAULT_SIPARIS_CONFIG, type SiparisThemeConfig } from '@/lib/menuSiparisConfig';

export default function SiparisThemeSettingsModal({
  open,
  initial,
  saving,
  onClose,
  ...
  open: boolean;
  initial: SiparisThemeConfig;
  saving?: boolean;
  onClose: () => void;
  onSave: (config: SiparisThemeConfig) => Promise<void> | void;
}) {
  const [draft, setDraft] = useState<SiparisThemeConfig>(initial);
  ...
  return (
    <div
      className="linear-settings siparis-settings"
      role="dialog"
      aria-modal="true"
      aria-labelledby="siparis-settings-title"
    >
      <button type="button" className="linear-settings__scrim" aria-label="Kapat" onClick={onClose} />
      <div className="linear-settings__panel">
        <header className="linear-settings__head linear-settings__head--siparis">
          <div>
            <p className="linear-settings__eyebrow">
              <Settings2 className="w-3.5 h-3.5" /> Sipariş Odaklı
            </p>
            <h2 id="siparis-settings-title">Tema ayarları</h2>
        ...
        <div className="linear-settings__body siparis-settings__body">
          <button
            type="button"
          ...
          <p className="siparis-settings__hint">
            Kapalıysa ürün detayında boy / ekstra / istek seçenekleri gizlenir. Varsayılan:{' '}
            {DEFAULT_SIPARIS_CONFIG.variantsEnabled ? 'açık' : 'kapalı'}.
          </p>
          ...
          <p className="siparis-settings__hint">
            Kapalıysa sepete ekleme ve sepet ikonu gizlenir. Varsayılan:{' '}
            {DEFAULT_SIPARIS_CONFIG.cartEnabled ? 'açık' : 'kapalı'}.
          </p>
