...
export default function LuxuryProductOptions({
  groups,
  basePrice,
  currency,
  onChange,
  ...
  return (
    <section className="lux-opts lux-detail__reveal" aria-label="Ürün seçenekleri">
      <header className="lux-opts__head">
        <h2>Seçenekler</h2>
        <p>Boy, ekstra ve isteklerini buradan seç.</p>
      </header>

      <div className="lux-opts__groups">
        {activeGroups.map((g) => {
          ...
          if (g.type === 'single') {
            ...
            return (
              <div key={g.id} className="lux-opts__group">
                <div className="lux-opts__label">
                  <span>
                    {g.name}
                    {g.required ? <i aria-hidden>*</i> : null}
                  </span>
                </div>
                <div className="lux-opts__chips" role="radiogroup" aria-label={g.name}>
                  {!g.required ? (
                    <button
                      type="button"
                      role="radio"
                      aria-checked={!picks.length}
                      className={`lux-opts__chip${!picks.length ? ' is-on' : ''}`}
                      onClick={() => {
                  ...
                  {visible.map((o) => {
                    ...
                    return (
                      <button
                        ...
                        role="radio"
                        aria-checked={on}
                        className={`lux-opts__chip${on ? ' is-on' : ''}`}
          ...
          return (
            <div key={g.id} className="lux-opts__group">
              ...
              <ul className="lux-opts__multi">
                {visible.map((o) => {
                  const cur = picks.find((x) => x.optionId === o.id);
                  const q = cur?.qty || 0;
                  const priceText = optionPriceLabel(g, o.price, currency);
                  return (
                    <li key={o.id} className="lux-opts__multi-row" data-sold-out={o.stockQty === 0 ? 'true' : undefined}>
                      <div className="lux-opts__multi-copy">
                        <strong>{o.name}{o.stockQty === 0 ? ' · bitti' : ''}</strong>
                        {priceText ? <span>{priceText}</span> : null}
                      </div>
                      <div className="lux-opts__stepper">
                        <button
                          type="button"
      ...
      <div className={`lux-opts__total${priceChanged ? ' is-live' : ''}`}>
