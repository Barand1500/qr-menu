...
export default function StandartHome({
  ...
  return (
    <div className="std-home">
      ...
      {/* PC: koyu kategori ızgarası */}
      <nav className="std-cats-grid" aria-label={en ? 'Categories' : 'Kategoriler'}>
        {menu.groups.map((g) => (
          <Link key={g.id} to={menuGroupPath(g.id)} className="std-cat-card">
            <span className="std-cat-card__media">
              {g.imageUrl ? (
                <img src={imageUrl(g.imageUrl)} alt="" />
              ) : (
                <MenuMediaPlaceholder kind="group" size="lg" label={g.name} />
              )}
            </span>
            <span className="std-cat-card__name">{g.name}</span>
            {g.productCount != null ? (
              <span className="std-cat-card__count">{g.productCount}</span>
            ) : null}
          </Link>
        ))}
      </nav>
