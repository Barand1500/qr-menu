export const MENU_LINEAR_CONFIG_KEY = 'menu_linear_config';
  ...
  try {
    ...
    const features: LinearFeatureLine[] = [0, 1, 2, 3].map((i) => {
      ...
      return {
        ...
        text: typeof row?.text === 'string' && row.text.trim() ? row.text.trim().slice(0, 80) : fallback.text,
      };
    });
    return {
      headline:
...
export function serializeLinearThemeConfig(config: LinearThemeConfig): string {
  return JSON.stringify(parseLinearThemeConfig(JSON.stringify(config)));
}
