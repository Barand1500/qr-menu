export const WELCOME_BASKETBALL_CONFIG_KEY = 'welcome_basketball_config';
...
export function parseWelcomeBasketballConfig(
  ...
  try {
    data = raw ? (JSON.parse(raw) as Partial<WelcomeBasketballConfig>) : {};
  } catch {
    data = {};
  }
...
export function serializeWelcomeBasketballConfig(config: WelcomeBasketballConfig) {
